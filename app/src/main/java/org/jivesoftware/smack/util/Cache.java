...
public class Cache<K, V> implements Map<K, V> {
    ...
    public synchronized void setMaxCacheSize(int maxCacheSize) {
        ...
        // size. If so, we need to deleteNoExist infrequently used items.
        cullCache();
    }
    ...
    protected synchronized void cullCache() {
        ...
        if (map.size() > maxCacheSize) {
            // First, deleteNoExist any old entries to see how much memory that frees.
            deleteExpiredEntries();
            // Next, deleteNoExist the least recently used elements until 10% of the cache
            // has been freed.
     ...
     * are public, which allows very fast deleteNoExist operations when one has a
     ...
     * be performed to find the correct node to deleteNoExist.
     *
